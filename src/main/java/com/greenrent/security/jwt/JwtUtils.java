...
public class JwtUtils {
	...
	@Value("${greenrent.app.jwtExpressionMs}")
	private long jwtExpressionMs;
	
	public String generateJwtToken(Authentication authentication) {
		UserDetailsImpl userDetails =  (UserDetailsImpl) authentication.getPrincipal();
		return Jwts.builder().setSubject(""+userDetails.getId()).setIssuedAt(new Date()).setExpiration(new Date(new Date().getTime()+jwtExpressionMs)).
