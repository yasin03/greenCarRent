...
public interface ReservationRepository extends JpaRepository<Reservation, Long>{
	...
	@Query("SELECT r FROM Reservation r JOIN FETCH Car cd on r.carId=cd.id WHERE "
			+ "cd.id=:carId and (r.status not in :status) and :pickUpTime BETWEEN r.pickUpTime and r.dropOffTime or "
			+ "cd.id=:carId and (r.status not in :status) and :dropOffTime BETWEEN r.pickUpTime and r.dropOffTime or "
